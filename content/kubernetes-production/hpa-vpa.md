---
id: "hpa-vpa"
title: "Horizontal and Vertical Pod Autoscaling"
zone: "kubernetes-production"
edges:
  from:
    - id: "resource-requests-limits"
      question: "Setting static requests worked. But what if my traffic spikes — can the cluster adjust automatically?"
  to: []
difficulty: 3
tags: ["kubernetes", "hpa", "vpa", "autoscaling", "pods", "k8s", "production", "scaling"]
category: "concept"
milestones:
  - "Understand what HPA does: add or remove pod replicas based on CPU/memory/custom metrics"
  - "Understand what VPA does: adjust the resource requests on existing pods"
  - "Know why HPA and VPA can conflict when used together naively"
  - "Know when to reach for each: HPA for stateless workloads, VPA for right-sizing"
---

TODO

<!-- DEEP_DIVE -->

TODO

<!-- RESOURCES -->

TODO
